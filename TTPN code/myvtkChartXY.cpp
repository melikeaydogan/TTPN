...
//-----------------------------------------------------------------------------
vtkStandardNewMacro(myvtkChartXY);

//-----------------------------------------------------------------------------
myvtkChartXY::myvtkChartXY()
{
...
//-----------------------------------------------------------------------------
bool myvtkChartXY::Paint(vtkContext2D *painter)
{
...
//-----------------------------------------------------------------------------
void myvtkChartXY::CalculateBarPlots()
{
...
//-----------------------------------------------------------------------------
void myvtkChartXY::RecalculatePlotTransforms()
{
...
//-----------------------------------------------------------------------------
int myvtkChartXY::GetPlotCorner(vtkPlot *plot)
{
...
//-----------------------------------------------------------------------------
void myvtkChartXY::SetPlotCorner(vtkPlot *plot, int corner)
{
...
//-----------------------------------------------------------------------------
void myvtkChartXY::RecalculatePlotBounds()
{
...
//-----------------------------------------------------------------------------
bool myvtkChartXY::UpdateLayout(vtkContext2D* painter)
{
...
//-----------------------------------------------------------------------------
int myvtkChartXY::GetLegendBorder(vtkContext2D* painter, int axisPosition)
{
...
//-----------------------------------------------------------------------------
void myvtkChartXY::SetLegendPosition(const vtkRectf& rect)
{
...
//-----------------------------------------------------------------------------
vtkPlot * myvtkChartXY::AddPlot(int type)
{
...
//-----------------------------------------------------------------------------
vtkIdType myvtkChartXY::AddPlot(vtkPlot * plot)
{
  if (plot == NULL)
    {
    return -1;
...
//-----------------------------------------------------------------------------
bool myvtkChartXY::RemovePlot(vtkIdType index)
{
...
//-----------------------------------------------------------------------------
vtkPlot* myvtkChartXY::GetPlot(vtkIdType index)
{
...
//-----------------------------------------------------------------------------
void myvtkChartXY::SetShowLegend(bool visible)
{
...
//-----------------------------------------------------------------------------
vtkChartLegend* myvtkChartXY::GetLegend()
{
  return this->Legend;
}

//-----------------------------------------------------------------------------
vtkTooltipItem* myvtkChartXY::GetTooltip()
{
  return this->Tooltip;
}

//-----------------------------------------------------------------------------
vtkIdType myvtkChartXY::GetNumberOfPlots()
{
  return this->ChartPrivate->plots.size();
}

//-----------------------------------------------------------------------------
vtkAxis* myvtkChartXY::GetAxis(int axisIndex)
{
  if (axisIndex < 4)
    {
...
//-----------------------------------------------------------------------------
vtkIdType myvtkChartXY::GetNumberOfAxes()
{
  return 4;
}
...
//-----------------------------------------------------------------------------
void myvtkChartXY::RecalculateBounds()
{
  // Ensure that the bounds are recalculated
  this->PlotTransformValid = false;
  // Mark the scene as dirty
...
//-----------------------------------------------------------------------------
bool myvtkChartXY::Hit(const vtkContextMouseEvent &mouse)
{
...
//-----------------------------------------------------------------------------
bool myvtkChartXY::MouseEnterEvent(const vtkContextMouseEvent &)
{
  chosenBar = -1;
...
//----------------------------------------------------------------------------- Li Liu
bool myvtkChartXY::MouseDoubleClickEvent(const vtkContextMouseEvent &mouse)
{
     ...
     pos[1] < this->Point2[1] && n)
  {
     for(size_t i = 0; i < this->ChartPrivate->PlotCorners.size(); ++i)
     {
         ...
         if(items)
         {
            ...
            vtkVector2f tolerance(5*(1.0/transform->GetMatrix()->GetElement(0, 0)), 5*(1.0/transform->GetMatrix()->GetElement(1, 1)));
            ...
            for(int j = items-1; j >= 0; --j)
            {
                ...
                if(plot && plot->GetVisible())
                {
                   ...
                   if(plotBar)
                   {
                     seriesIndex = plotBar->GetNearestPoint(position, tolerance, &plotPos, &segmentIndex);
                   }
                   else
                   {
                     seriesIndex = plot->GetNearestPoint(position, tolerance, &plotPos);
                   }
                 }
             }
          }
  ...
  if (mouse.GetButton() == vtkContextMouseEvent::RIGHT_BUTTON)
  {
    ...
    //cout << x << " " << y << " " << endl;
    //cout << seriesIndex << endl;
    chosenBar = seriesIndex;
    return true;
  }
...
//-----------------------------------------------------------------------------
bool myvtkChartXY::MouseMoveEvent(const vtkContextMouseEvent &mouse)
{
...
//-----------------------------------------------------------------------------
bool myvtkChartXY::LocatePointInPlots(const vtkContextMouseEvent &mouse,
                                    int invokeEvent)
{
  size_t n = this->ChartPrivate->plots.size();
  vtkVector2i pos(mouse.GetScreenPos());
...
void myvtkChartXY::SetTooltipInfo(const vtkContextMouseEvent& mouse,
                                ...
                                vtkIdType segmentIndex)
{
  ...
  vtkVector2d temp_plot = vtkVector2d (double(plotPos.GetX()), double(plotPos.GetY()));
  vtkStdString tooltipLabel = plot->GetTooltipLabel(temp_plot, seriesIndex,
                                                    segmentIndex);
...
//-----------------------------------------------------------------------------
bool myvtkChartXY::MouseLeaveEvent(const vtkContextMouseEvent &)
{
  this->DrawNearestPoint = false;
  this->Tooltip->SetVisible(false);
  return true;
...
//-----------------------------------------------------------------------------
bool myvtkChartXY::MouseButtonPressEvent(const vtkContextMouseEvent &mouse)
{
  chosenBar = -1;
  this->Tooltip->SetVisible(false);
  // Iterate through each corner, and check for a nearby point
...
/*
bool myvtkChartXY::MouseButtonReleaseEvent(const vtkContextMouseEvent &mouse)
{
  // Iterate through each corner, and check for a nearby point
  for (size_t i = 0; i < this->ChartPrivate->PlotCorners.size(); ++i)
    {
...
void myvtkChartXY::ZoomInAxes(vtkAxis *x, vtkAxis *y, float *origin, float *max)
{
...
//-----------------------------------------------------------------------------
bool myvtkChartXY::MouseWheelEvent(const vtkContextMouseEvent &, int delta)
{
  this->Tooltip->SetVisible(false);
...
/*
bool myvtkChartXY::KeyPressEvent(const vtkContextKeyEvent &key)
{
...
//-----------------------------------------------------------------------------
bool myvtkChartXY::RemovePlotFromCorners(vtkPlot *plot)
{
...
//-----------------------------------------------------------------------------
void myvtkChartXY::PrintSelf(ostream &os, vtkIndent indent)
{
